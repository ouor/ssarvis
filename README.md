...
- 회원 인증 및 소유권 분리
  ...
  - 친구 요청을 보내고, 수락/거절/취소/해제 가능
  - 친구가 되면 서로의 비공개 클론과 음성을 조회하고 사용할 수 있음
  - 클론과 음성은 기본 비공개이며, 소유자가 공개로 전환하면 다른 로그인 사용자도 조회하고 사용할 수 있음
...
1. Friends 탭에서 사용자 검색 후 친구 요청 전송
2. 상대는 받은 요청을 수락하거나 거절 가능
3. 친구가 되면 각자 `friend` scope로 상대의 비공개 클론과 음성을 조회 가능
4. 친구 자산도 채팅과 논쟁에 바로 사용할 수 있음
5. 친구 해제 후에는 기존 기록은 유지되지만, 새 친구 기반 접근은 차단됨
...
친구 자산이나 공개 자산을 사용해 시작한 대화도 저장 주체는 현재 로그인 회원입니다.
...
친구 자산이나 공개 자산을 사용해 시작한 논쟁도 저장 주체는 현재 로그인 회원입니다.
...
- 공개 자산 사용 후 비공개 재전환 시 기존 기록 유지 / 신규 사용 차단 검증
- 친구 수락 전 접근 불가 / 수락 후 사용 가능 / 친구 해제 후 신규 사용 차단 검증
...
- 공개 자산은 소유자가 비공개로 되돌릴 수 있으며, 이 경우 기존 채팅/논쟁 기록은 유지되지만 새 사용은 차단됩니다.
- 친구 자산은 친구 관계가 유지되는 동안에만 새로 사용할 수 있으며, 친구 해제 후에는 기존 기록만 계속 볼 수 있습니다.
