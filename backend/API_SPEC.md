...
- `promptGenerationLogId`, `conversationId`, `registeredVoiceId`는 모두 현재 로그인한 회원 소유여야 한다.
...
- `topic`은 비어 있으면 안 된다.
- `cloneAId`, `cloneBId`, `cloneAVoiceId`, `cloneBVoiceId`는 모두 현재 로그인한 회원 소유여야 한다.
