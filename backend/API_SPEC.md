...
- 친구 요청이 수락되면 친구의 비공개 클론과 음성도 조회하고 사용할 수 있다.
- 친구 관계가 해제되면 기존에 생성된 채팅/논쟁 기록은 유지되고, 이후 새 친구 기반 접근만 차단된다.
...
친구 관계는 `friend_requests` 테이블로 관리한다.
...
- 이미 친구인 사용자에게는 새 요청을 보낼 수 없다.
- 같은 사용자 쌍에 대해 `PENDING` 요청은 한 번만 존재할 수 있다.
- 상대가 이미 나에게 `PENDING` 요청을 보낸 상태에서 다시 요청하면, 서버는 수락하라는 안내와 함께 `409 Conflict`를 반환한다.
- 거절되었거나 취소된 요청은 다시 보낼 수 있다.
...
- `receiverUserId`가 없거나 자기 자신에게 요청한 경우

#### `409 Conflict`

- 이미 친구인 경우
- 같은 방향 요청이 이미 pending인 경우
- 상대가 이미 나에게 pending 요청을 보낸 경우
...
현재 로그인한 회원이 받은 pending 친구 요청 목록을 반환한다.
...
Body
- `POST /api/friends/requests` 응답 배열

## GET `/api/friends/requests/sent`

현재 로그인한 회원이 보낸 pending 친구 요청 목록을 반환한다.
...
Body
- `status`가 `ACCEPTED`로 바뀐 친구 요청 응답

## POST `/api/friends/requests/{requestId}/reject`

받은 pending 친구 요청을 거절한다.
...
Body
- `status`가 `REJECTED`로 바뀐 친구 요청 응답

## POST `/api/friends/requests/{requestId}/cancel`

내가 보낸 pending 친구 요청을 취소한다.
...
Body
- `status`가 `CANCELED`로 바뀐 친구 요청 응답

## GET `/api/friends`

현재 로그인한 회원의 친구 목록을 반환한다.
...
[
  {
    ...
    "friendsSince": "2026-03-27T03:00:00Z"
  }
]
```
...
현재 로그인한 회원과 지정한 사용자의 친구 관계를 해제한다.

설명
- 현재 `ACCEPTED` 상태의 친구 관계를 종료 상태로 바꾼다.
- 기존에 그 친구 자산으로 생성한 채팅/논쟁 기록은 유지된다.
- 해제 이후에는 새 친구 기반 접근만 차단된다.
...
## GET `/api/friends/users/search`

활성 사용자 중 현재 로그인한 회원 자신을 제외하고 검색한다.
...
- `scope=friend`이면 현재 로그인 회원의 친구가 소유한 비공개 클론만 반환된다.
- `scope=public`이면 공개된 클론만 반환된다.
- 친구 클론과 공개 클론은 서로 다른 목록으로 분리된다.
...
- `scope=friend`이면 현재 로그인 회원의 친구가 소유한 비공개 음성만 반환된다.
- `scope=public`이면 공개된 음성만 반환된다.
- 친구 음성과 공개 음성은 서로 다른 목록으로 분리된다.
